...
-- 6. Habilitar RLS na tabela registros
...
CREATE POLICY "registros_select_policy" ON registros
    FOR SELECT USING (
        ...
        OR
        -- Fretista: pode ver todos os registros (sem filtro por usuário específico por enquanto)
...
-- 11. Habilitar RLS em outras tabelas do sistema (apenas se existirem)
-- Descomente as linhas abaixo se as tabelas existirem no seu banco
-- ALTER TABLE clientes ENABLE ROW LEVEL SECURITY;
-- ALTER TABLE fretistas ENABLE ROW LEVEL SECURITY;
-- ALTER TABLE emitentes ENABLE ROW LEVEL SECURITY;

-- 12. Policies genéricas para outras tabelas (descomente se as tabelas existirem)
...
FROM pg_policies 
WHERE tablename IN ('usuarios', 'registros')
